...
const Lesson: React.FC = () => {
  ...
  const category = useMemo(() => {
    if (id === 'bookmarks') {
      const bookmarkedIds = new Set(getBookmarks());
      // Optimized filtering using Set lookup (O(1)) and pre-calculated cards list
      const bookmarkedCards = allCards.filter(c => bookmarkedIds.has(c.id));
