...
The Tagalog Learning App is designed with a "Mobile-First" and "Performance-First" philosophy. To achieve a smooth 60 FPS experience on both high-end and low-end devices, several critical optimizations have been implemented across the codebase.
...
### Search and Indexing
The Home page uses a module-level `lessonsWithSearchIndex` that contains pre-lowercased Tagalog and English fields. This significantly speeds up the filtering of hundreds of lessons during user search by eliminating the overhead of repeated `toLowerCase()` calls and property lookups.
