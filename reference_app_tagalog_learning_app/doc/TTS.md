...
The Tagalog Learning App provides a comprehensive Text-to-Speech (TTS) system to help users learn correct pronunciations. The system is designed to be highly reliable and performant, utilizing a multi-layered fallback chain and an efficient caching mechanism.
...
5. **Web Speech API**: As a final resort, the system utilizes the device's native `SpeechSynthesis` capabilities (if available for the Tagalog language).
