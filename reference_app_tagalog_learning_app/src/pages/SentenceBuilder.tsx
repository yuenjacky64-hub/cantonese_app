...
const SentenceBuilder: React.FC = () => {
    ...
    const allExampleCards = useMemo(() => {
        const cards = lessons.flatMap(l => l.cards).filter(c => c.example && c.example.tagalog.trim().length > 0);
        // Filter out examples that are too short (less than 3 words)
        return cards.filter(c => c.example!.tagalog.split(' ').length >= 3);
    }, []);
    ...
    const startGame = () => {
        ...
        const mappedData: SentenceGameData[] = shuffled.map(card => {
            const sentence = card.example!.tagalog;
